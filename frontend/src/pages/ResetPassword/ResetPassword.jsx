...
const ResetPassword = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!token) {
      setErrors({ form: 'Missing or invalid recovery token. Please request a new link.' });
      return;
    }
    ...
    if (handleValidation()) {
      ...
      try {
        ...
        if (res.success) {
          ...
          setIsLoading(false);
          setErrors({ form: res.message || 'Token expired or invalid.' });
        }
      } catch (err) {
        setIsLoading(false);
            ...
            {isSuccess ? (
              ...
              >
                <div style={{ display: 'inline-flex', justifyContent: 'center', alignItems: 'center', width: '56px', height: '56px', borderRadius: '50%', backgroundColor: '#10B981', marginBottom: '1.5rem' }}>
                  <img src={arrowRightPng} alt="success" style={{ width: '20px', height: '20px', transform: 'rotate(90deg)' }} />
                </div>
              ...
              >
                <motion.div variants={elementVariants} style={{ marginBottom: '1.5rem' }}>
                  ...
                  <p style={{ color: '#666666', fontSize: '0.9rem' }}>
                    Type your new secure password below to update your account.
                  </p>
                </motion.div>
                ...
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
                  ...
                  <motion.div variants={elementVariants} className="form-group">
                    ...
                    <div style={{ position: 'relative' }}>
                      ...
                      >
                        {showPassword ? (
                          eyeOffPng && <img src={eyeOffPng} alt="" style={{ width: '18px', height: '18px' }} />
                        ) : (
                          eyePng && <img src={eyePng} alt="" style={{ width: '18px', height: '18px' }} />
                        )}
                      </button>
                    </div>
                    {errors.password && (
